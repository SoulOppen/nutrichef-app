...
import {
  callGeminiAPI, compactProfile, buildExploreCacheKey,
  buildGeneratorRecipeCacheKey, EXPLORE_CACHE_KEY, GENERATOR_RECIPE_CACHE_KEY,
} from '../lib/gemini.js';
...
export default function ExploreView() {
  ...
  // Recetas destacadas filtradas por el perfil del usuario (Kosher, dieta, etc.)
  const featuredRecipes = getFeaturedRecipes(profile, 6).map(r => ({
    id: r.title,
    name: r.title,
    type: r.cuisine,
  ...
  const handleSearch = async () => {
    ...
    // 1. Buscar local con filtros de dieta/Kosher aplicados
    const localResults = searchLocalRecipes(query, profile);
    ...
    // 2. No hay resultados locales — mostrar botón de escape a IA
  ...
  const handleSearchWithAI = async () => {
    ...
    const dietNote = profile.religiousDiet !== 'Ninguna'
      ? ` IMPORTANTE: El usuario tiene dieta ${profile.religiousDiet}. Solo sugerir recetas compatibles.`
      : '';

    const prompt = `El usuario busca: "${query}". Genera 3 opciones de recetas adaptadas a su perfil: ${profileStr}${favoriteRecipes.length > 0 ? `. Le gustan: ${favoriteRecipes.map(r => r.title).join(', ')}` : ''}.${dietNote}
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-8">
      ...
      <div className="p-8 rounded-3xl shadow-md text-white text-center" style={{ background: 'linear-gradient(135deg, #6366f1, #7c3aed)' }}>
        ...
        <p className="text-indigo-100 mb-1 max-w-lg mx-auto text-sm">
          Escribe un plato, ingrediente o lo que se te antoje.
        </p>
        {profile.religiousDiet !== 'Ninguna' && (
          <p className="text-indigo-200 text-xs mb-4">
            🔍 Filtrando automáticamente para <strong>{profile.religiousDiet}</strong>
          </p>
        )}
        ...
        <div className="flex gap-2 bg-white/10 p-2 rounded-2xl backdrop-blur-md max-w-2xl mx-auto border border-white/20">
          <input
            ...
            placeholder='Ej: "shakshuka", "pollo", "snack proteico"...'
      ...
      {/* Sin resultados locales — botón de escape a IA */}
      {noLocalResults && !loading && (
        <div className="bg-white dark:bg-gray-900 rounded-2xl border border-dashed border-indigo-200 dark:border-indigo-800 p-8 text-center space-y-4 animate-in fade-in">
          <p className="text-slate-600 dark:text-slate-300 font-semibold">
            No encontramos "<span className="text-indigo-600 dark:text-indigo-400">{query}</span>" en nuestro banco de recetas.
          </p>
          <p className="text-slate-400 dark:text-slate-500 text-sm">
            {profile.religiousDiet !== 'Ninguna'
              ? `Solo mostramos recetas compatibles con ${profile.religiousDiet}.`
              : 'Puedes pedir a la IA que cree una receta única para ti.'}
          </p>
          <button
            onClick={handleSearchWithAI}
            className="mx-auto flex items-center gap-2 px-6 py-3 rounded-2xl text-white font-bold text-sm transition-all"
            style={{ background: 'linear-gradient(135deg, #6366f1, #7c3aed)' }}
          >
            <Sparkles size={18} /> ✨ Crear receta única con IA
          </button>
          <p className="text-xs text-slate-400 dark:text-slate-500">
            La IA generará una receta personalizada para tu perfil. Se guardará en tu historial.
          </p>
        </div>
      )}
      ...
      {suggestions && !recipe && !generatingRecipe && (
        <div className="space-y-4 animate-in fade-in slide-in-from-bottom-4">
          <div className="flex items-center justify-between">
            ...
            {/* Botón de escape a IA si hay resultados locales pero el usuario quiere más */}
            {sourceLabel === 'local' && (
              <button
                onClick={handleSearchWithAI}
                ...
                <div className="flex items-start justify-between mb-2">
                  ...
                  {sugg._local && (
                    <div className="flex items-center gap-1">
                      <Zap size={11} className="text-green-500" title="Sin IA" />
                      {sugg._local?.isKosher && (
                        <span className="text-[9px] font-black text-blue-600 bg-blue-100 dark:bg-blue-900/30 px-1.5 py-0.5 rounded-full">Kosher</span>
                      )}
                    </div>
                  )}
                </div>
      ...
      {recipe && (
        <div>
          <button
            ...
            className="mb-4 text-indigo-600 dark:text-indigo-400 font-medium flex items-center gap-1 hover:underline text-sm"
          >
      ...
      {/* Estado vacío — featured recipes filtradas por perfil */}
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            <h3 className="text-sm font-black uppercase tracking-widest text-slate-400 dark:text-slate-500 flex items-center gap-2">
              ...
              {profile.religiousDiet !== 'Ninguna'
                ? `Recetas ${profile.religiousDiet} — sin IA`
            ...
            {profile.religiousDiet !== 'Ninguna' && (
              <span className="text-xs font-bold text-blue-600 dark:text-blue-400 bg-blue-50 dark:bg-blue-900/20 px-2 py-1 rounded-full">
                Filtrado: {profile.religiousDiet}
              </span>
            )}
          </div>
              ...
              >
                <div className="flex items-start justify-between mb-1">
                  ...
                  <div className="flex gap-1">
                    ...
                    {sugg._local?.isKosher && (
                      <span className="text-[9px] font-black text-blue-500">K</span>
                    )}
                  </div>
                </div>
          ...
          <p className="text-xs text-center text-slate-400 dark:text-slate-500">
            Busca arriba para más recetas, o escribe lo que quieres para que la IA lo cree.
          </p>
        </div>
      )}
    </div>
