...
export default function GeneratorView() {
  ...
  const getSuggestions = async () => {
    ...
    // FIX: ahora la cacheKey incluye dishType, difficulty y cuisine
    const suggestionsCacheKey = buildGeneratorSuggestionsCacheKey({
      ...
      profile: {
        ...profile,
        favoriteTitles: favoriteRecipes.map((r) => r.title),
      }
    });
    ...
    const prompt = `
      ...
      - Platos que le encantan: ${favoriteRecipes.length > 0 ? favoriteRecipes.map(r => r.title).join(', ') : 'Aún ninguno'}
      - Preferencias aprendidas por IA: ${profile.learnedPreferences.length > 0 ? profile.learnedPreferences.join(' | ') : 'Ninguna'}

      Genera 3 OPCIONES diferentes de preparaciones que se puedan hacer con esos ingredientes.
      ...
      <div className="lg:col-span-4 space-y-6">
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-orange-100">
          ...
          <div className="space-y-4">
            ...
            {cacheNotice && (
              <div className="rounded-xl border border-blue-200 bg-blue-50 p-3 text-sm text-blue-900">{cacheNotice}</div>
            )}
              ...
              {suggestions.map((sugg) => (
                <div key={sugg.id} className="bg-white p-6 rounded-2xl shadow-sm border border-orange-100 flex flex-col sm:flex-row gap-6 items-start sm:items-center hover:border-orange-300 transition-colors">
                  <div className="flex-1">
                    <span className="text-xs font-bold text-orange-600 bg-orange-100 px-3 py-1 rounded-full inline-block mb-2">{sugg.type}</span>
                  ...
                  <button
                    ...
                    className="w-full sm:w-auto py-2.5 px-6 bg-orange-50 text-orange-700 font-bold rounded-xl hover:bg-orange-600 hover:text-white transition-colors flex items-center justify-center gap-2 shrink-0 disabled:opacity-60"
                  >
