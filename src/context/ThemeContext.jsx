...
export function ThemeProvider({ children }) {
  const [themeId, setThemeId] = useState(() => localStorage.getItem('nutrichef_theme') || 'orange');
  // null = seguir sistema, 'light' = forzar claro, 'dark' = forzar oscuro
  const [colorMode, setColorMode] = useState(() => localStorage.getItem('nutrichef_colormode') || null);
  ...
  const setTheme = (id) => {
    setThemeId(id);
    localStorage.setItem('nutrichef_theme', id);
  };

  const setMode = (mode) => {
    setColorMode(mode);
    if (mode === null) localStorage.removeItem('nutrichef_colormode');
    else localStorage.setItem('nutrichef_colormode', mode);
  };
